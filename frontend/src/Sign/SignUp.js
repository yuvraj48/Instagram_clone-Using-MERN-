import React, { useState } from 'react';
import { Link } from 'react-router-dom';
import { Button, Form, Input,Row,Col } from 'reactstrap';
import {signup} from "./Helper"

const SignUp = () => {
  const [values,setValues]=useState({
    name:"",
    lastname:"",
    email:"",
    password:"",
    error:"",
    success:""
  });

  const {name,lastname,email,password,error,success}=values;

  const handleChange=name=>event=>{
    setValues({...values,error:false,[name]:event.target.value});
  };

  const onSubmit=event=>{
    event.preventDefault();
    setValues({...values,error:false});
    signup({name,lastname,email,password})
    .then(data=>{
      if(data&&data.error){
        setValues({ ...values, error: data.error, success: false });
      }
      else{
        setValues({
          ...values,
          name: "",
          lastname:"",
          email: "",
          password: "",
          error: "",
          success: true
        });
      }
    });

  };
  const successMessage = () => {
    return (
      <div className="row">
        <div className="col-md-12 ">
          <div
            className="alert alert-success  border-0 text-center"
            style={{ display: success ? "" : "none" }}
          >
            New account was created successfully. Please{" "}
            <Link to="/login">Login Here</Link>
          </div>
        </div>
      </div>
    );
  };

  const errorMessage = () => {
    return (
      <div className="row ">
        <div className="col-md-12  text-center">
          <div
            className="alert alert-danger border-0"
            style={{ display: error ? "" : "none" }}
          >
            {error}
          </div>
        </div>
      </div>
    );
  };
  
  


 const signupform=()=>{
  return (
      
    <div className="container   border bg-white " style={{maxWidth:"350px"}}>
      {successMessage()}
      {errorMessage()}
    <Form className="ml-5 mr-5"  >
        <img src="data:image/png;base64,"
        alt="loading" style={{maxWidth:"250px",marginTop:"30px"}} className="ml-auto mr-auto col-sm-3" />

        

       <Row className="text-center mt-2">
           <Col md={12} >
           <h6 className="text-secondary">Sign Up to see photos and videos from your friends </h6>
           </Col>
           
        </Row>
        

      <Row className="mt-3">
        <Col md={12} className="ml-auto mr-auto">
        <Input type="email" name="email"  placeholder="Email" className="form-control form-control-sm bg-light" onChange={handleChange("email")} value={email}/>
        </Col>
      </Row>

      <Row className="mt-1">
        <Col md={12}  className="ml-auto mr-auto">
        <Input type="text"   placeholder="Fullname" className="form-control form-control-sm bg-light" onChange={handleChange("name")} value={name} />
        </Col>
      </Row>

      <Row className="mt-1" >
        <Col md={12} className="ml-auto mr-auto">
        <Input type="text"  placeholder="lastname" className="form-control form-control-sm bg-light" onChange={handleChange("lastname")} value={lastname}/>
        </Col>
      </Row>

      <Row className="mt-1">
        <Col md={12} className="ml-auto mr-auto">
        <Input type="text"  placeholder="Password" className="form-control form-control-sm bg-light" onChange={handleChange("password")} value={password} />
        </Col>
      </Row>

     
      <Button type="submit" color="primary" size="sm" className="mt-3 col-12" onClick={onSubmit}>Sign UP</Button>

      <Row className="mb-4">
          <Col md={12}>
          <p className="text-secondary mt-3 " style={{fontSize:"12px"}}>By signing you are agree to our Terms and cookie policy </p>
          </Col>
      </Row>
    </Form>
  
    </div>
  );
}
return (
    <div>
        <div className="mt-5">
        {signupform()}
        <div className="border container  text-center mt-3  bg-white p-3" style={{maxWidth:"350px"}}>
            <p>Don't have an account ? <Link to = "/login">Login</Link></p>
            

        </div>
        </div>

    </div>
)
}

export default SignUp;