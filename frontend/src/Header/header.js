import React, { useState ,useEffect} from 'react'
import HomeIcon from '@material-ui/icons/Home';
import {  Avatar, IconButton } from '@material-ui/core'
import { Navbar,Nav,NavItem ,Button,Modal, ModalBody} from 'reactstrap';
import { NavLink, useHistory,Link, withRouter } from 'react-router-dom';
import { isAutheticated, signout } from '../Sign/Helper';
import FavoriteBorderIcon from '@material-ui/icons/FavoriteBorder';
import ExitToAppIcon from '@material-ui/icons/ExitToApp';
import SearchIcon from '@material-ui/icons/Search';
import AddAPhotoIcon from '@material-ui/icons/AddAPhoto';

function Header() {
     const {user,token}=isAutheticated();
     const [modal,setModal]=useState(false);
     const toggle=()=>setModal(!modal);
    const [search,setSearch] = useState('')
    const [userDetails,setUserDetails] = useState([])
    const [picture,setPicture] = useState([])
    let history = useHistory();
    
   


    useEffect(() => {
      if(token)
      fetch(`/user/${user._id}`,{
          headers:{
              "Content-Type":"application/json",
              Authorization:`Bearer ${token}`
          }
      }).then(res=>res.json())
      .then(result=>{
          setPicture(result.pic);
          
          

      })
      
  }, [])

    const fetchUsers = (query)=>{
        setSearch(query)
        fetch(`/searching`,{
          method:"post",
          headers:{
            "Content-Type":"application/json"
          },
          body:JSON.stringify({
            query
          })
        })
        .then(res=>res.json())
        .then(results=>{
         setUserDetails(results.user)
        })
     }
    return (
        <div className="bg-white mt-0 mb-1" style={{maxHeight:"60px" }}>
         {isAutheticated()&&(
             <Navbar className=" container mt-0" >
                 <img src="data:image/png;base64,"
        alt="loading" style={{maxWidth:"110px"}} className="mr-auto " />

        <div className="nav">
            <div className="navitem">
              <form className="d-none d-sm-block d-md-block form-inline ">
                <input type="search" id="form1"   value={search}
                onChange={(e)=>fetchUsers(e.target.value)} className="form-control form-control-sm modal-trigger" placeholder="Search "aria-label="Search" />
                <Button className="btn btn-sm bg-light text-secondary"  onClick={toggle}><SearchIcon style={{fontSize:"18px"}}/></Button>

                <Modal isOpen={modal} toggle={toggle}  style={{maxWidth:"300px",overflow:"scroll"}}>
                 <ModalBody>
                   <ul className="text-center" style={{listStyleType:"none",padding:"0",justifyContent:"space-between"}}>
                     {userDetails.map(item=>{
                       return <Link to={item._id !== user._id ? "/profile/"+item._id:'/profile'} 
                       ><li style={{display:"flex",color:"black",padding:"10px"}} > <Avatar src={item.pic.slice(-1)[0]} style={{width:"24px", height:"25px"}}/><h6 className="ml-md-3">{item.name}</h6></li></Link> 
                       })}
                    </ul>
                 </ModalBody>
                </Modal>
                                       
              </form>

            </div>

        </div>
        
        <Nav className="ml-auto  ">
          <NavItem>
              <NavLink to="/">
                  <IconButton>
                      <HomeIcon/>
                  </IconButton>
              </NavLink>
              </NavItem>

             
              <NavItem>
              <NavLink to="/profile">
              {!picture.length==0?
              <IconButton>                    
               <Avatar src={picture.slice(-1)[0] }  style={{width:"25px",height:"25px" ,borderRadius:"80px"}} alt="..."/>                   
              </IconButton>:
              <IconButton>                    
                <Avatar   style={{width:"25px",height:"25px" ,borderRadius:"80px"}} alt="..."/>                   
              </IconButton>

            }
                          
              </NavLink>
              </NavItem>
              <NavItem>
              <NavLink to="/newpost">
                  <IconButton>
                      <AddAPhotoIcon/>
                  </IconButton>
              </NavLink>
              </NavItem>

              <NavItem>             
                   <span
               className="nav-link text-warning "
               onClick={()=>{
                 signout(()=>{
                   history.push("/login")
                 })
               }}
              
              ><ExitToAppIcon/>
              </span>
            
              </NavItem>
              
              

              <NavItem></NavItem>
            
           
              </Nav>
              </Navbar>
            )}


        </div>
    )
}

export default withRouter(Header)
